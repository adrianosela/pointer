...
func Test_Value(t *testing.T) {
	...
	t.Run("Value with non nil string", func(t *testing.T) {
		str := "string"
		assert.Equal(t, str, Value(&str))
	})
	t.Run("Value with non nil map", func(t *testing.T) {
		m := map[string]string{"string": "string"}
		assert.Equal(t, m, Value(&m))
	})
	t.Run("Value with non nil struct", func(t *testing.T) {
		s := struct{ Field string }{Field: "string"}
		assert.Equal(t, s, Value(&s))
	})
	t.Run("Value with non nil generic interface", func(t *testing.T) {
		var i interface{} = 10
		assert.Equal(t, i, Value(&i))
	})
	t.Run("Value with nil boolean", func(t *testing.T) {
		var b *bool
		zeroValueOfBool := false
		assert.Equal(t, zeroValueOfBool, Value(b))
	})
	t.Run("Value with nil string", func(t *testing.T) {
		var str *string
		zeroValueOfString := ""
		assert.Equal(t, zeroValueOfString, Value(str))
	})
	t.Run("Value with nil map pointer", func(t *testing.T) {
		var m *map[string]string
		zeroValueOfMapPointer := map[string]string(nil)
		assert.Equal(t, zeroValueOfMapPointer, Value(m))
	})
	t.Run("Value with nil struct pointer", func(t *testing.T) {
		type anon struct {
			Field string
		}
		var s *anon
		zeroValueOfStruct := anon{}
		assert.Equal(t, zeroValueOfStruct, Value(s))
	})
	t.Run("Value with nil generic interface", func(t *testing.T) {
		var i *interface{}
		zeroValueOfGenericInterface := interface{}(nil)
		assert.Equal(t, zeroValueOfGenericInterface, Value(i))
	})
}
